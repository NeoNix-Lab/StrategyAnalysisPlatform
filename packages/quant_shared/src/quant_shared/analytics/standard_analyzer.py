...
from quant_shared.analytics.regime_performance import RegimePerformanceAggregator
import pandas as pd
import numpy as np
...
class StandardAnalyzer:
    ...
    def calculate_portfolio_metrics(self, strategy_id: str = None, run_id: str = None) -> dict:
        ...
        """
        df = self._build_trade_dataframe(strategy_id=strategy_id, run_id=run_id)
        if df.empty:
        ...
        regime_performance = RegimePerformanceAggregator.aggregate(df)
        ...
        return {
            ...
            "avg_fill_latency": round(avg_fill_latency, 3),
    ...
    def _empty_metrics(self):
        return {
            ...
            "pnl_skew": 0,
            "pnl_kurtosis": 0,
            "regime_performance": RegimePerformanceAggregator.empty(),
            "equity_curve": []
        }
    ...
    def calculate_regime_performance(self, strategy_id: str = None, run_id: str = None) -> dict:
        df = self._build_trade_dataframe(strategy_id=strategy_id, run_id=run_id)
        return RegimePerformanceAggregator.aggregate(df)

    def _build_trade_dataframe(self, strategy_id: str = None, run_id: str = None) -> pd.DataFrame:
