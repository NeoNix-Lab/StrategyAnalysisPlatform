...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "& { . .venv/Scripts/Activate.ps1; cd services/api_gateway; $env:TRADING_DB_PATH='$dbPath'; $env:PYTHONPATH='$PWD/src'; python -m uvicorn src.api.main:app --reload --port 8000 }"

# 10. ML Core (Port 5000)
Write-Host "Starting ML Core (Port 5000)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "& { . .venv/Scripts/Activate.ps1; cd services/ml_core; $env:TRADING_DB_PATH='$dbPath'; $env:PYTHONPATH='$PWD/src'; python -m uvicorn src.main:app --reload --port 5000 }"
